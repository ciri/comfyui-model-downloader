...
These workflows are intentionally small, public, and credential-free. Load one by dragging its `.json` file into ComfyUI, then queue it. They download into the selected ComfyUI model directory.

| Workflow | What it verifies | Download size |
| --- | --- | --- |
| `huggingface-tiny-download.json` | Hugging Face download and in-node progress | about 520 KB |
| `civitai-tiny-download.json` | CivitAI metadata lookup, download, and API-key-free public access | about 250 bytes |
| `auto-model-finder-scan.json` | Workflow scan and Hugging Face lookup | no download |
